...
function AllColdproducts() {
  ...
  var deletehotpro = async (id) => {
      try {
          var response = await fetch(`http://localhost:5001/colddelete/${id}`, {
              method: "DELETE"
          });
    ...
    <div>
       <div className='d-flex'>
                ...
                <div className='container'>
                    <h2 className='text-center-mainheading'>SHOW ALL NEW-PRODUCTS</h2>
                            ...
                            <div className='container'>
                                <table className='table table-bordered '>
                                    ...
                                    {InputFieldname.map((item) => (
                                                <tr>
                                                    <td>{item.tital}</td>
                                                    <td><img src={`http://localhost:5001/showcoldpro/${item.image}`} style={{ maxHeight :"80px"} } /></td>
                                                    <td>{item.price}</td>
                                                    <td>{item.pricecut}</td>
                                                    <td>
...
export default AllColdproducts